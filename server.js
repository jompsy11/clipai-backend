...
// Called by clipai-ten.vercel.app when user pastes a YouTube URL
app.post('/api/youtube-upload', async (req, res) => {
  ...
  exec(cmd, { maxBuffer: 1024 * 1024 * 200, timeout: 600000 }, async (err, stdout, stderr) => {
    ...
    if (!ASSEMBLYAI_KEY) {
      const uploadUrl = `https://${req.headers.host}/api/serve-upload/${localFileId}`;
      return res.json({ localFileId, uploadUrl });
    }
    ...
    try {
      ...
      const uploadUrl = `https://${req.headers.host}/api/serve-upload/${localFileId}`;
      res.json({ localFileId, uploadUrl });
    }
  });
});
